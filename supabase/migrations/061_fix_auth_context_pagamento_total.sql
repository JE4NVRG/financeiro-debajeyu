-- Migration: Fix authentication context for pagamento_total
-- Problema: O sistema usa autenticação customizada (não Supabase Auth), então auth.uid() sempre retorna NULL
-- Solução: Modificar função para receber user_id como parâmetro em vez de usar auth.uid()

-- Recriar a função process_pagamento_total para receber user_id como parâmetro
...
BEGIN
    ...
    INSERT INTO pagamentos_fornecedores (
        ...
        p_user_id  -- Usar parâmetro fornecido em vez de auth.uid()
...
-- Também atualizar process_pagamento_parcial para consistência
...
COMMENT ON FUNCTION process_pagamento_total IS 'Função corrigida para usar user_id como parâmetro em vez de auth.uid() - compatível com autenticação customizada';
COMMENT ON FUNCTION process_pagamento_parcial IS 'Função corrigida para usar user_id como parâmetro em vez de auth.uid() - compatível com autenticação customizada';